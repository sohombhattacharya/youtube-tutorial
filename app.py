...
def convert_html_to_pdf():
    ...
    html_content = f"<style>body {{ font-size: 150%; }}</style>" + \
                   f"<p>Generated by <a href='https://swiftnotes.ai' target='_blank'>swiftnotes.ai</a></p>\n" + \
                   (f"<p>YouTube Link: <a href='{youtube_url}' target='_blank'>{youtube_url}</a></p>\n" if youtube_url else "") + \
                   html_content
    
    # Ensure all hyperlinks in the HTML open in a new tab
    html_content = re.sub(r'<a\s+(?![^>]*target=)[^>]*>', lambda m: m.group(0) + ' target="_blank"', html_content)
